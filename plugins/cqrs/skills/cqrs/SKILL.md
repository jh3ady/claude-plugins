---
name: cqrs
description: This skill should be used when separating the model that handles commands (writes) from the model that answers queries (reads), designing command or query handlers, shaping a read model or projection differently from the write model, deciding whether the read and write paths should diverge, or reviewing a system that splits reads from writes, applying CQRS even when the term is not used. Also applies to simple CRUD, to judge whether CQRS belongs there at all.
---
