...
description: This skill should be used when implementing a feature, fixing a bug, or changing behaviour, before writing the implementation, and whenever writing or designing a test, deciding what to test, driving a use case or domain rule test-first, weighing a mock against a stub or an in-memory fake, or finding a test that breaks after a refactor that changed no behaviour, applying classical (Detroit-style) test-driven development even when "TDD" is not said. Not a testing-tooling helper: runners, assertion libraries, coverage, and CI setup are out of scope.
---
