...
description: This skill should be used when deciding the shape and balance of an automated test suite, choosing at which level to test something, weighing unit against integration against end-to-end tests, judging a test pyramid, testing trophy, or inverted ice-cream cone, placing consumer-driven contract tests, reading or setting a coverage target, or dealing with flaky tests, applying testing strategy even when the term is not named. This is the outer loop: the inner red-green-refactor loop belongs to test-driven-development.
---
