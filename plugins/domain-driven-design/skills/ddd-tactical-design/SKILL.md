...
description: Apply the tactical design half of domain-driven design (DDD) when modelling domain building blocks, deciding whether a concept is an entity or a value object, designing an aggregate and its consistency boundary, selecting a repository or a domain service, reviewing code for an anemic domain model, or expressing a rich domain inside one bounded context, even when the terms "DDD", "aggregate", "entity", or "value object" are not used. Also covers Vernon's four aggregate design rules, the repository-versus-DAO distinction, the domain-service-versus-application-service distinction, domain events as first-class building blocks, factories for complex creation, and modules as ubiquitous-language partitions. Also applies when a request to apply domain-driven design does not specify whether strategic or tactical concerns are in scope, because modelling the building blocks that express a rich domain inside a bounded context is the complementary second half of any DDD engagement alongside strategic design.
---
