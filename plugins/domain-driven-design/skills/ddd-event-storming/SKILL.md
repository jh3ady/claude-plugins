...
description: This skill should be used when running or preparing a collaborative domain-discovery workshop, exploring a complex or unfamiliar business process with domain experts, mapping domain events on a timeline, hunting for candidate bounded contexts and aggregates, or turning workshop sticky notes into a domain model, applying EventStorming even when the term is not used. Also applies whenever a domain is poorly understood and should be explored collaboratively before drawing boundaries or writing code.
---
