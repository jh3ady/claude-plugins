---
name: event-sourcing
description: This skill should be used when designing, reviewing, or refactoring how part of a system persists its state as a history of events rather than a current-state snapshot, when working with an append-only event store or event streams, when an aggregate is rebuilt by replaying events, when building projections or read models from events, or when an audit trail, temporal queries, or reconstructing past state is needed, applying event sourcing even when the term is not used. Also applies to simple CRUD, to judge whether event sourcing belongs there at all.
---
