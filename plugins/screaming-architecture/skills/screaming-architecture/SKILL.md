...
description: This skill should be used when judging whether a codebase's top-level layout makes its business domain legible rather than merely announcing its framework, when a newcomer cannot tell what a system does from its directory tree, when choosing between organising code by technical layer and by feature, use case, or vertical slice, or when reviewing a folder or project structure for what it reveals about the domain, applying Screaming Architecture even when the term is not used. For the direction of dependencies use hexagonal or clean architecture; for module boundary enforcement use the modular monolith.
---
